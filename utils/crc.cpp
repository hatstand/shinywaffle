#include <cstdio>
#include <functional>

using std::extent;

static const uint16_t CRC16_POLY = 0x8005;
static const uint16_t CRC_INIT = 0xffff;

static const uint8_t CHECKSUM[] = { 0xcd, 0xac };
static const uint8_t DATA[] = {
  0x09, 0x57, 0x16, 0x0a, 0x2b, 0x76, 0x09, 0x31, 0x22, 0x13,
};

// http://www.ti.com/lit/an/swra322/swra322.pdf
static const uint8_t PN9[] = {
  ,
};

// http://www.ti.com/lit/an/swra111d/swra111d.pdf
uint16_t crc(uint8_t data, uint16_t crc_reg) {
  for (int i = 0; i < 8; ++i) {
    if (((crc_reg & 0x8000) >> 8) ^ (data & 0x80)) {
      crc_reg = (crc_reg << 1) ^ CRC16_POLY;
    } else {
      crc_reg = (crc_reg << 1);
    }
    data <<= 1;
  }
  return crc_reg;
}

uint8_t pn9_table(uint8_t data, int* state) {
  uint8_t out = data ^ PN9[*state];
  *state = (++*state) % std::extent<decltype(PN9)>::value;
  return out;
}

int main(int argc, char** argv) {
  uint16_t checksum = CRC_INIT;
  uint16_t pn9_checksum = CRC_INIT;

  int state = 0;
  char pn9_data[extent<decltype(DATA)>::value];
  for (int i = 0; i < extent<decltype(DATA)>::value; ++i) {
    pn9_data[i] = pn9_table(DATA[i], &state);
  }

  for (int i = 0; i < extent<decltype(DATA)>::value; ++i) {
    checksum = crc(DATA[i], checksum);
    pn9_checksum = crc(pn9_data[i], pn9_checksum);
  }

  printf("CRC()      %x\n", checksum);
  printf("CRC(PN9()) %x\n", pn9_checksum);

  return 0;
}
